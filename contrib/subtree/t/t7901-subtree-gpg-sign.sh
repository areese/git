...
# Usage: test_signed_commit [options] <message> [<file> [<contents> [<tag>]]]
#   -C <dir>:
...
test_create_pre2_32_repo () {
	...
	test_signed_commit -C "$1" --gpg-sign main1 &&
	test_signed_commit -C "$1-sub" --gpg-sign --annotate sub1 &&
	...
	test_signed_commit -C "$1-sub" --gpg-sign --annotate sub2 &&
...
test_validate_all_commits_are_signed () {
	test_expect_code 1 git -C "$1" log --pretty='format:%G?'  | awk '/E/ { exit 1 }'
}
...
test_expect_success GPGSSH 'push basic operation' '
	...
	(
		cd "$test_count" &&
		git fetch ./"sub proj" HEAD &&
		git subtree add --gpg-sign --ggp-sign --prefix="sub dir" FETCH_HEAD
	) &&
	test_create_commit "$test_count" "sub dir"/main-sub1 &&
	test_create_commit "$test_count" main2 &&
	test_create_commit "$test_count/sub proj" sub2 &&
	...
	(
		cd "$test_count" &&
		git fetch ./"sub proj" HEAD &&
		git subtree merge --gpg-sign --ggp-sign --prefix="sub dir" FETCH_HEAD &&
		before=$(git rev-parse --verify HEAD) &&
		split_hash=$(git subtree split --gpg-sign --prefix="sub dir") &&
		git subtree push --ggp-sign --prefix="sub dir" ./"sub proj" from-mainline &&
